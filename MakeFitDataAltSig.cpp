...
std::pair<float, float> MakeFitDataAltSig(TTree* tree, std::tuple<float,float,float,float,float,float,float,float,float> dInfo, bool DoPlot, std::string FigName,bool fixedMass)
{
  ...
  RooRealVar a0("a0", "a0", std::get<6>(dInfo),std::get<6>(dInfo)*1.025,std::get<6>(dInfo)*0.975);
  RooRealVar a1("a1", "a1", std::get<7>(dInfo),std::get<7>(dInfo)*0.975,std::get<7>(dInfo)*1.025);
  float a2f,a2t;
  if(std::get<8>(dInfo)>0){
    a2f=std::get<8>(dInfo)*0.975;
    a2t=std::get<8>(dInfo)*1.025;
  }else{
    a2f=std::get<8>(dInfo)*1.025;
    a2t=std::get<8>(dInfo)*0.975;
  }
  RooRealVar a2("a2", "a2", std::get<8>(dInfo),a2f,a2t);
  ...
  if(DoPlot)
    {
      ...
      RooPlot* massframe = Diele_mass.frame(Title("exp (x) gauss"));
