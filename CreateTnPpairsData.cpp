...
void CreateTnPpairsData() 
{
  ...
  *mvaEleID_Fall17_iso_V2_wpHZZ_unsopported=0,*mvaEleID_Fall17_iso_V2_wp80=0,*mvaEleID_Fall17_iso_V2_wp90=0;
  ...
  TFile* file = TFile::Open("/eos/user/n/nstrautn/Bparking_DATA/Bpark_22_DM_test_DATA.root"); // Change input DATA ntuple file here (Ntuple should come from the related miniAOD Ntuplizer)
  ...
  TTree* selectedTree = originalTree->CopyTree("(ElectronMVAEstimatorRun2Fall17NoIsoV2Values[0]>0.95 && ele_pt[0]>7 && (fabs(ele_eta[0])<1.44 || fabs(ele_eta[0])>1.57)) || (ElectronMVAEstimatorRun2Fall17NoIsoV2Values[1]>0.95 && ele_pt[1]>7 && (fabs(ele_eta[1])<1.44 || fabs(ele_eta[1])>1.57))");
  ...
  TTree* TempTree = originalTree->CopyTree("(ElectronMVAEstimatorRun2Fall17NoIsoV2Values[0]>0.95 && ele_pt[0]>7 && (fabs(ele_eta[0])<1.44 || fabs(ele_eta[0])>1.57)) && (ElectronMVAEstimatorRun2Fall17NoIsoV2Values[1]>0.95 && ele_pt[1]>7 && (fabs(ele_eta[1])<1.44 || fabs(ele_eta[1])>1.57))");
  ...
  for(Long64_t i=0;i<nentries;i++) //Tag-Tag pairs in which second is selected as tag
  {
    ...
    BranchProbeChIso_->Fill();
    BranchProbeNeuIso_->Fill();
    BranchProbePhoIso_->Fill();
